...
@WebServlet("/School_ManagerFormAction3.do")
public class School_ManagerFormAction3 extends HttpServlet {
    public void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        String college_name = new String(request.getParameter("college_name").getBytes("ISO-8859-1"), "UTF-8").trim();
        ...
        //System.out.println(collegeName+" "+status+" "+content+" "+scope+" "+major_name+" "+class_name+" "+number);
        if(status.equalsIgnoreCase("学生")){
            StudentDao sd=new StudentDao();
            if(content.equalsIgnoreCase("健康码查询")){
                if(scope.equalsIgnoreCase("按个人查询")){ //按个人查询
                    Student stu=sd.findBySno(number);
                    ...
                    if(scope.equalsIgnoreCase("按学院查询"))
                        array_stu=sd.findStudentsByCollege(college_name);
                    else if(scope.equalsIgnoreCase("按专业查询")){
                        array_stu=sd.findStudentsByMajorAndCollege(major_name,college_name);
                    }
                    else if(scope.equalsIgnoreCase("按班级查询")){
                        array_stu=sd.findStudentsByClassAndMajorAndCollege(class_name,major_name,college_name);
                    }
                    StringBuffer results=new StringBuffer("<table>");
                    results.append("<tr><td>学号</td><td>姓名</td><td>健康码</td></tr>");
                ...
                if(scope.equalsIgnoreCase("按学院查询")){
                    array_ss=ssd.findStudentsByCollege(college_name);

                }
                else if(scope.equalsIgnoreCase("按专业查询")){
                    array_ss=ssd.findStudentsByCollegeAndMajor(college_name,major_name);
                }
                else if(scope.equalsIgnoreCase("按班级查询")){
                    array_ss=ssd.findStudentsByCollegeAndMajorAndClass(college_name,major_name,class_name);
                }
                else{ //按个人查询
            ...
            if(content.equalsIgnoreCase("健康码查询")){
                if(scope.equalsIgnoreCase("按个人查询")){ //按个人查询
                    Teacher tea=td.findByTno(number);
                    //System.out.println(stu.getSno());
                    StringBuffer results=new StringBuffer("<table>");
                    ...
                    if(scope.equalsIgnoreCase("按学院查询"))
                        array_tea=td.findTeachersByCollege(college_name);
                    //System.out.println(array_tea.size());
                    StringBuffer results=new StringBuffer("<table>");
                    results.append("<tr><td>工号</td><td>姓名</td><td>身份</td><td>健康码</td></tr>");
                ...
                if(scope.equalsIgnoreCase("按学院查询")){
                    array_ts=tsd.findTeachersByCollege(college_name);
                }
                else{ //按个人查询
                    array_ts=tsd.findTeachersByTno(number);
                }
                StringBuffer results=new StringBuffer("<table>");
